...
**Goal:** Build a single-file React artifact where users can add cars and track oil changes, tire rotations, and other service intervals by both mileage and date, with localStorage persistence and PWA push notifications.

**Architecture:** Single `CarMaintenance.jsx` file containing all data types, localStorage helpers, status calculation logic, and UI components. No external dependencies beyond what's already in the project (React, lucide-react, Tailwind). Push notifications use the browser Web Push API — permission is requested once on first open, and due/overdue items fire a notification on each app load.

**Tech Stack:** React 18, Tailwind CSS, lucide-react, localStorage, Web Notifications API
...
|------|--------|----------------|
| `src/artifacts/CarMaintenance.jsx` | Create | Entire artifact — data model, localStorage, status logic, all UI |
| `src/App.jsx` | Modify | Add import + `/car-maintenance` route |
| `src/hub/Hub.jsx` | Modify | Add Car Maintenance entry to APPS array |
...
```js
// Stored in localStorage under key "car-maintenance-v1"
// Shape: Car[]

// Car
{
  id: string,           // crypto.randomUUID()
  nickname: string,     // e.g. "Jaymes's Truck"
  year: string,
  make: string,
  model: string,
...
// Service
{
  ...
  lastServiceDate: string | null,  // ISO date string "YYYY-MM-DD"
}

// Status: "green" | "yellow" | "red"
// yellow = due within 500 miles OR 30 days
// red    = overdue (past interval)
```

## Default Services (reference for all tasks)
...
## Task 1: Scaffold CarMaintenance.jsx with data layer

**Files:**
- Create: `src/artifacts/CarMaintenance.jsx`

- [ ] **Step 1: Create the file with imports, constants, data helpers, and status logic**
...
- [ ] **Step 2: Register route and hub entry**

In `src/App.jsx`, add after the Pomodoro import:
```jsx
import CarMaintenance from "./artifacts/CarMaintenance.jsx";
```

Add after the `/pomodoro` route:
```jsx
<Route path="/car-maintenance" element={<CarMaintenance />} />
```

In `src/hub/Hub.jsx`, add to the APPS array after the tamagotchi entry:
```js
{
...
```bash
cd /home/jaymes/github-repos/artifacts && npm run dev
```

Navigate to http://localhost:5173/car-maintenance — should show "Car Maintenance — scaffold" on a dark background. Hub should show the 🚗 tile.

- [ ] **Step 4: Commit**

```bash
git add src/artifacts/CarMaintenance.jsx src/App.jsx src/hub/Hub.jsx
git commit -m "feat: scaffold car maintenance artifact with data layer"
```

---

## Task 2: Hub view — car list

**Files:**
- Modify: `src/artifacts/CarMaintenance.jsx`

- [ ] **Step 1: Replace the placeholder `return` in CarMaintenance with the hub view**

Replace the entire `return (...)` block in `CarMaintenance` with:
...
The hub view should show "No cars yet" empty state with a Car icon. "Add Car" button should be visible but can't navigate yet (will be wired in Task 3).

- [ ] **Step 3: Commit**

```bash
git add src/artifacts/CarMaintenance.jsx
git commit -m "feat: car maintenance hub list view"
```

---

## Task 3: Add / Edit car modal

**Files:**
- Modify: `src/artifacts/CarMaintenance.jsx`

- [ ] **Step 1: Add CarForm component above the CarMaintenance function**
...
- [ ] **Step 2: Wire add-car and edit-car views into CarMaintenance**

Inside the `CarMaintenance` function, add these handlers before the `if (view === "hub")` block:
...
Add these view branches before the final `return` fallback:
...
Tap "Add Car" — modal slides up from bottom. Fill in a car name and mileage, tap "Add Car" — should navigate to the car detail view (still shows "Loading..." for now, that's fine). Reload the page — car persists in the list.

- [ ] **Step 4: Commit**

```bash
git add src/artifacts/CarMaintenance.jsx
git commit -m "feat: add/edit car form with localStorage persistence"
```

---

## Task 4: Car detail view — service list

**Files:**
- Modify: `src/artifacts/CarMaintenance.jsx`

- [ ] **Step 1: Add StatusBadge helper component above CarMaintenance**
...
- [ ] **Step 2: Add serviceSubtitle helper above CarMaintenance**
...
- [ ] **Step 3: Add the car detail view branch inside CarMaintenance**

Add this block after the `edit-car` view branch and before the final fallback return:
            ...
            {selectedCar.services.map(service => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-3 bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 text-left w-full active:scale-98 transition-transform"
                >
...
- [ ] **Step 4: Add MileageUpdater component above CarMaintenance**
...
Add a car → car detail view shows all 10 services with yellow badges (no history). Mileage "update" link opens an inline editor. Edit pencil goes to edit form. Back link returns to hub.

- [ ] **Step 6: Commit**

```bash
git add src/artifacts/CarMaintenance.jsx
git commit -m "feat: car detail view with service list and mileage updater"
```

---

## Task 5: Log service view

**Files:**
- Modify: `src/artifacts/CarMaintenance.jsx`

- [ ] **Step 1: Add LogServiceForm component above CarMaintenance**
...
- [ ] **Step 2: Wire log-service view and handler into CarMaintenance**

Add handler inside `CarMaintenance` alongside the other handlers:
...
Add view branch after the `edit-car` branch:
...
Open a car, tap a service row → log-service modal slides up. Pre-fills today's date and current mileage. Save — returns to car detail, service row now shows date and next-due mileage. Badge turns green if well within interval.

- [ ] **Step 4: Commit**

```bash
git add src/artifacts/CarMaintenance.jsx
git commit -m "feat: log service form wired to car detail"
```
...
- [ ] **Step 1: Add notification helpers above CarMaintenance**
...
- [ ] **Step 2: Add notification permission prompt component above CarMaintenance**
...
- [ ] **Step 3: Wire notifications into CarMaintenance**

Add state at the top of `CarMaintenance`:

```jsx
  const [showNotifPrompt, setShowNotifPrompt] = useState(false);
```

Add a `useEffect` after the save effect:
...
In the hub view return, add the prompt overlay just before the closing `</div></div>`:
...
Add a car, log a service as overdue (set lastServiceDate a year ago, lastServiceMileage well below current). Reload the page — browser should show a notification prompt (or fire immediately if already granted). Accept the prompt — notification should appear for the overdue service.

Note: Notifications only fire on HTTPS or localhost. They won't fire on a plain HTTP deploy.

- [ ] **Step 5: Commit**

```bash
git add src/artifacts/CarMaintenance.jsx
git commit -m "feat: push notifications for due and overdue services"
```

---

## Task 7: Polish and .gitignore
...
```bash
echo ".superpowers/" >> /home/jaymes/github-repos/artifacts/.gitignore
```

- [ ] **Step 2: Add "no services due" empty state to car detail**

In the car detail service list section, after the `</div>` that closes the services list, add:
...
- [ ] Add two cars with different makes
- [ ] Log services on one car — verify badge turns green
- [ ] Set a service mileage so it's within 500 miles of due — verify yellow badge
- [ ] Reload — all data persists
- [ ] Hub shows correct status badges per car

- [ ] **Step 4: Final commit**

```bash
git add src/artifacts/CarMaintenance.jsx .gitignore
git commit -m "chore: polish car maintenance — empty states, gitignore .superpowers"
```
